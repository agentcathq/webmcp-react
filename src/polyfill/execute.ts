...
export function runTool(
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    // Call tool.execute synchronously (not deferred behind an extra
    // microtask) so it registers its own abort listener on `controller.signal`
    // before a caller abort dispatched in the same synchronous turn can reach
    // it. A synchronous throw is caught and routed through the same failure
